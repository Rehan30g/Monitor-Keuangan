    ...
    db.exec(`
      INSERT INTO users_rebuild
        (id, username, email, passwordHash, createdAt, emailVerified,
         failedLoginCount, lockedUntil, displayName, avatarExt, role, deletedAt, google_id)
...
// Bootstrap admin: promosikan username yang terdaftar di ADMIN_USERNAMES
// (dipisah koma) menjadi role='admin' setiap startup. Idempoten & self-healing
// (hanya promosi, tak pernah demosi otomatis — demosi hanya lewat panel admin).
// process.env sudah terisi: ./env.js di-import paling atas file ini.
try {
  const raw = process.env.ADMIN_USERNAMES || '';
  const usernames = raw
    .split(',')
    .map((u) => u.trim().toLowerCase()) // samakan dengan normalizeUsername()
    .filter(Boolean);
